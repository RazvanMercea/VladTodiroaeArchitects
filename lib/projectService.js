...
export const updateProjectInDatabase = async (projectId, updatedData) => {
  try {
    ...
    const existingProject = existingProjectSnap.exists()
    ...
    const imageUrls = await Promise.all(
      updatedData.images.map(async (img) => {
        if (img.url) return img.url; // already hosted
        return await uploadFileAndGetUrl(
          `projects/${projectId}/images/${img.file.name}`,
          img.file
        );
      })
    );

    // Upload new plans only
    const planUrls = await Promise.all(
      Object.keys(updatedData.plans).map(async (floorType) => {
        ...
        const url = await uploadFileAndGetUrl(
          `projects/${projectId}/plans/${plan.file.name}`,
          plan.file
        );
        return { floorType, url };
      })
...
export const deleteProjectFromDatabase = async (projectId) => {
  try {
    ...
    // Delete all files from Storage (both images and plans)
    await deleteAllFilesInFolder(`projects/${projectId}`);
